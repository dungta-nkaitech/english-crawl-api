  ...
  for (const fileName of files) {
    ...
    try {
      ...
      try {
        ...
        console.warn(`⚠️  No transcript found for ${fileName}, skipping...`);
        continue;
      }
      ...
      const vocabItems: IVocabItem[] = (meta.vocabItems || []).map((v) => ({
        id: uuidv4(),
        episodeId: episodeId,
        word: v.word,
      ...
      const transcripts: ITranscript[] = transcriptData.map((t, index) => ({
        id: uuidv4(),
        episodeId: episodeId,
        order: index,
        text: t.text,
